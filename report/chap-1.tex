...
In order to be able to explain the averaged observed feature vector, the weights $\bm{\theta}$ need to be found that match the expected features vector obtained from the distribution $\bm{F}(r_{expected})$ with the averaged observed features vector $\bm{F}_{av}$. $\bm{r}_{expected}$ defined as $ E(p(\bm{r}|\bm{\theta}))$. To make the expected features vector match the averaged observed features vector, gradient descent method can be used with as gradient $\bm{F}_{av} - \bm{F}(r_{expected})$. Equation \ref{eq:5} summarizes the gradient descent method.
\begin{equation}\label{eq:5}
	\bm{\theta}^{k+1} = \bm{\theta}^{k} - (\bm{F}_{av}^k - \bm{F}(r_{expected})^k) 
\end{equation}
