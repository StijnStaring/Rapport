...
\begin{flushleft}
  ...
  \begin{tabularx}{\textwidth}{@{}p{12mm}X@{}}
    ...
    $\bm{F}_{obs}(\bm{r})$ & Observed feature vector $\in \mathbb{R}^{Z\times 1}$ with on its entries the different feature values for the observed path $\bm{r}$.\\
    $\bm{F}_{diff}$ & The difference between the observed and expected feature vector: $F_{obs}(\bm{r}) - F(\bm{r})$.\\
