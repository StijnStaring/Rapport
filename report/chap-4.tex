...
\end{figure}
 \newpage                                
\section{Conclusion}
In this chapter the learning of driver specific comfort parameters with a realistic $15$ dof Amesim model was discussed. The flow of the different steps of the learning process and how the Amesim model would be embedded, was looked into. The question posed was: "Is it possible to find back the underlying comfort weights when the gradient $\pdv{\bm{F}_{diff}}{\bm{\theta}}$ is estimated by $ \bm{F}^* - \bm{F}(\bm{r}_{expected})$". \\
