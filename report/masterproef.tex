...
In order to validate the developed algorithms for learning driver specific weighting factors, data is generated with a known objective function from where the weighting factors are tried to find back. Implementations are attained that make use of a non-linear bicycle model and a more complex $15$ degrees of freedom Amesim model. Learning is done making use of gradient descent, wherefore an alternative method is proposed based on an optimized weighting factors step size.\\

Results were retrieved that show that the lateral weighting factors, that determine a lane change, were accurately learned. Due to a good match between learned and observed feature values, the observed kinematic vehicle signals were adequately learned. This research was performed with the help of Siemens Digital Industries Software. \\
\end{abstract}
