...
\setup{% Verwijder de "%" op de volgende lijn bij UTF-8 karakterencodering
  ...
  assessor={Prof.\,dr.\,ir.\ Herman Bruyninckx \and Dr.\,ir.\ Bert Pluymers},
...
The human driver transforms from being an active traffic participant into a passive agent due to the developments in autonomous driving. Consequently, a motorist involvement becomes limited to perceiving and rating decisions taken by the vehicle. In order to enhance user acceptance and to increase competitiveness, it is important that a notion of driver specific comfort is considered i.e. trade-offs made between different comfort contributors. \\

Therefore, the objective of this research is to develop an algorithm making use of feature-based inverse optimal control which can learn driver specific experiences of comfort during a lane change, captured in weighting factors of a comfort objective function. Learning is done by looking at observations performed by the human driver because these are carried out based on an internal unknown objective function to generate comfortable paths. The retrieved objective function after learning, will approximate the real one used by the human agent.\\ Comfort features, which are integrated kinematic vehicle signals in a formulation so that they determine a notion of comfort, are used in order to quantify similarity between observed and learned vehicle paths. The contribution of this thesis is to implement the theoretical idea of feature-based learning with practical relevant vehicle models i.e. a $15$ degrees of freedom Amesim model. When the objective function is identified, it can be embedded in a motion planning formulation for autonomous driving.\\

In order to validate the developed algorithms for learning driver specific weighting factors, data is generated with a known objective function from which the weighting factors are tried to find back. Implementations are attained with a non-linear bicycle model and a more complex $15$ degrees of freedom Amesim model. Learning is done making use of gradient descent, wherefore an alternative method is proposed based on an optimized weighting factor update.\\

Results were retrieved that show that the lateral weighting factors, that determine a lane change, were accurately learned. Due to a good match between learned and observed feature values, the observed kinematic vehicle signals were also adequately learned. This research was performed with the help of Siemens Digital Industries Software. \\
\end{abstract}
