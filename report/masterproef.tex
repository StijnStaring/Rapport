...
The human driver transforms to an passive agent in autonomous driving and works as a black box that reacts on decisions taken by the autonomous vehicle that are based on delivering the human driver on its destination. In order to enhance user acceptance, it is important that a notion of driver specific comfort is taken into account. Therefore the objective of this research is to develop an algorithm based on inverse reinforced learning, that can learn driver specific experiences of comfort during a lane change captured in weights of an objective function. Comfort features, which are integrated kinematic vehicle signals that determine a notion of comfort, are used in order to quantify similarity between observed and learned vehicle paths. The contribution of this thesis is to implement the theoretical idea of feature based learning with practical relevant vehicle models e.g. a $15$ dof Amesim model. When the comfort objective is identified, it can be embedded in a path planning formulation.\\

In order to validate the developed algorithms to learn the different weights, data is generated with an known objective function which is tried to learn back. Algorithms to learn the weights of an unknown comfort objective function are developed using a non-linear bicycle model and a more complex $15$ dof Amesim model. Learning is done with gradient descent, wherefore an alternative weight update method is proposed.\\

Results were retrieved that show that the lateral features were accurately learned. Due to a good match between the lateral learned and observed feature values, the observed kinematic signals were adequately found back.\\
